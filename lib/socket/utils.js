  ...
  return function (event, channel, context, data) {
    ...
    channel.connections.forEach(connection => {
      ...
      if (socket) {
        ...
        const isHookEvent = context.service._hookEvents && context.service._hookEvents.includes(event);

        let result = channel.dataFor(connection) || context.dispatch || context.result;

        // If we are getting events from an array, and it is handled through the Feathers
        // event hook, try to get the individual item to dispatch from the correct index.
        if (isHookEvent && Array.isArray(context.result) && Array.isArray(result)) {
